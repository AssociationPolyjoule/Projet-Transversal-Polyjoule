...
float R=0.23714;//R=0.2364; //rayon roue
...
void calcul_vitesse(){
  ...
  if (etatPresent != etatPrecedent){ //Vérification d’un changement de bande
    ...
    if (Nv == 2*B) {  //Vérification des 3 tours de roue
      Tpresent = millis();
      Vit = (2*2*PI*R*3.6)/((Tpresent-Ttour)*0.001);//Calcul de la vitesse
      Ttour = millis();
      Nv = 0; //Remise à 0 du nombre de bande passées
    }
    else if (Vit > 13.0 and Vit<22 and Nv == B) {
      Tpresent = millis();
      Vit = (2*PI*R*3.6)/((Tpresent-Ttour)*0.001);//Calcul de la vitesse
      Ttour = millis();
      Nv = 0; //Remise à 0 du nombre de bande passées
    }
